...
const TIERS = [
  {
    ...
    blurb: "The full engine at small-team scale. Not a demo, not a trial.",
    points: [
      "All 7 differentiators — semantic + structural, anti-poisoning, reasoning reuse",
      "Up to 3 tenants · 100K cache entries",
      "Dashboard, Prometheus & OTel metrics",
      "Python + Node SDKs, crowkis cli, MCP server",
    ],
    cta: ["Pull the image", "/docker"],
    featured: false,
  },
  {
    name: "Enterprise",
    price: "Let's talk",
    cadence: "flat per cluster · priced on a call, not a meter",
    blurb: "Everything, ceilings removed, compliance included — and a human who picks up.",
    points: [
      "Unlimited tenants · virtual API keys with budgets & rate limits",
      "Crowkis Replay, Provider Arbitrage Router, cross-provider bridge",
  ...
  return (
    <section className="border-t-2 border-ink bg-paper-deep py-16 md:py-24">
      ...
      <Reveal>
        ...
        <h2 className="responsive-title mt-4 max-w-2xl">Free where it should be free.</h2>
        <p className="responsive-subtitle mt-4 max-w-2xl">
          One image, two tiers — a signed license file flips the unlock at boot. No phone-home,
          no usage metering, no surprise invoice. Flat per cluster, like infrastructure should be.
        </p>
      </Reveal>
          ...
          <Reveal key={tier.name} delay={i * 0.07} className="h-full">
            <article
              className={`flex h-full flex-col p-6 ${
                tier.featured ? "card-block !shadow-block-red" : "card-quiet"
              }`}
            >
              ...
              <p className="mt-4 font-display text-4xl font-bold tracking-tight">{tier.price}</p>
              <p className="mt-1 font-mono text-xs text-ink-faint">{tier.cadence}</p>
              <p className="mt-4 text-sm text-ink-soft">{tier.blurb}</p>
      ...
      <Reveal delay={0.15}>
        <p className="mt-8 text-center text-sm text-ink-faint">
          No DRM theatre. Same model as CockroachDB and Sentry: free for people who&apos;d never
          have paid, a contract for those who would.{" "}
          <Link href="/pricing" className="font-semibold text-crow underline underline-offset-2">
